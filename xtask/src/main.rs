...
use ast_src::{AstEnumSrc, AstNodeSrc, AstSrc, Cardinality, Field, KindsSrc, KINDS_SRC};
...
fn sourcegen_ast() {
    ...
    let grammar = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/typescript.ungram"))
        .parse()
        .unwrap();
...
fn generate_syntax_kinds(grammar: KindsSrc<'_>) -> String {
    ...
    let (keywords, keyword_idents): (Vec<_>, Vec<_>) = grammar
        .keywords
        .iter()
        .map(|(token, name)| (*token, format_ident!("{}", name)))
        .unzip();
    ...
    let ast = quote! {
        ...
        macro_rules! T {
            ...
            #([#keyword_idents] => { $crate::SyntaxKind::#keyword_idents };)*
        }
        pub use T;
    };
