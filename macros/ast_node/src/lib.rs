...
/// `type`s.
///
/// For example, `Lit` of ast is an enum, but `Expr`, which contains
    ...
    item = match input.data {
        Data::Enum(..) => {
            ...
            item.quote_with(smart_quote!(Vars { input }, {
                #[derive(::serde::Serialize, ::global_common::DeserializeEnum)]
                #[serde(untagged)]
                input
            }))
        }
...
///
///
/// TODO(swc): Delete works related to serde to `#[ast_serde]`.
#[proc_macro_attribute]
pub fn ast_node(
    args: proc_macro::TokenStream,
    input: proc_macro::TokenStream,
        ...
        Data::Enum(..) => {
            ...
            item.quote_with(smart_quote!(Vars { input }, {
                #[derive(
                    ...
                    ::global_common::Spanned,
                    Clone,
                    Debug,
                    PartialEq,
                    ::serde::Serialize,
                    ::global_common::DeserializeEnum,
                )]
                #[serde(untagged)]
                input
            }))
