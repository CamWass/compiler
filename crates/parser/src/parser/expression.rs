...
impl Parser<'_> {
    ...
    fn parse_assignment_expr_base(
        ...
        if self.ctx().in_generator() && self.is(tok!("yield")) {
    ...
    fn parse_primary_expr(&mut self, assign_props: &mut AssignProps) -> PResult<MaybeParen> {
        ...
        match self.input.cur() {
            ...
            tok!("import") => {
                ...
                if self.input.syntax().import_meta() && self.is(tok!('.')) {
                    return self
            ...
            tok!("async") => {
                if self.peeked_is(tok!("function"))
                ...
                if can_be_arrow && self.input.syntax().typescript() && self.peeked_is(tok!('<')) {
                    // try parsing `async<T>() => {}`
                ...
                if can_be_arrow && self.peeked_is(tok!('(')) {
                    expect!(self, "async");
    ...
    fn parse_subscript(
        ...
        if self.input.syntax().typescript() {
            ...
            if !matches!(obj, MaybeParenExprOrSuper::Super(_)) && self.is(tok!('<')) {
        ...
        let has_question_dot_token = if self.is(tok!('?')) && self.peeked_is(tok!('.')) {
            self.eat(tok!('?'));
            true
        } else {
            false
    ...
    pub(super) fn parse_lhs_expr(&mut self, assign_props: &mut AssignProps) -> PResult<MaybeParen> {
        ...
        let type_args = if self.input.syntax().typescript() && self.is(tok!('<')) {
            self.try_parse_ts(|p| {
                p.parse_ts_type_args()?;
                if p.is(tok!('(')) {
                    Ok(Some(()))
    ...
    fn parse_args_or_pats(
        ...
        while !self.is(tok!(')')) {
            if first {
                ...
                if current_item_has_spread && self.is(tok!(',')) {
            ...
            let mut arg = {
                if self.input.syntax().typescript()
                    && (self.is_ident_ref() || (self.is(tok!("...")) && self.peek_is_ident_ref()))
                {
                    let spread_start = if self.eat(tok!("...")) {
        ...
        if self.eat(tok!("new")) {
            ...
            if self.eat(tok!('.')) {
                if self.eat(tok!("target")) {
                    let span_of_target = self.input.prev_span();
    ...
    fn parse_paren_expr_or_arrow_fn(
        ...
        if self.syntax().typescript() && self.ctx().in_cond_expr() && self.is(tok!(':')) {
            // TODO: Remove clone
        ...
        if has_pattern || return_type.is_some() || self.is(tok!("=>")) {
...
impl Parser<'_> {
    fn parse_yield_expr(&mut self) -> PResult<Box<Expr>> {
        ...
        if self.is_semi_with_asi() || (!self.is(tok!('*')) && !self.input.cur().starts_expr()) {
