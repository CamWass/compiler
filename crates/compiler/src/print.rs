use ast::*;
use ecma_visit::{Visit, VisitWith, VisitAstNodeWith};
...
pub fn ast_graph<'ast, E>(
    ...
    FxHashMap<AstNode<'ast>, NodeIndex>,
    DiGraph<AstNode<'ast>, Edge<E>>,
)
where
    E: Debug + PartialEq + Clone + Copy,
{
...
where
    E: Debug + PartialEq + Clone + Copy,
{
    graph: DiGraph<AstNode<'ast>, Edge<E>>,
    map: FxHashMap<AstNode<'ast>, NodeIndex>,
    current_parent: Option<AstNode<'ast>>,
}
    ...
    E: Debug + PartialEq + Clone + Copy,
{
    ...
    pub fn create_node(&mut self, value: AstNode<'ast>) -> NodeIndex {
    ...
    pub fn create_edge(&mut self, from: AstNode<'ast>, branch: Edge<E>, to: AstNode<'ast>) {
    ...
    /// edge type.
    pub fn is_connected(&self, n1: NodeIndex, edge: Edge<E>, n2: NodeIndex) -> bool {
        self.graph
    ...
    pub fn connect_if_not_found(&mut self, n1: NodeIndex, edge: Edge<E>, n2: NodeIndex) {
        if !self.is_connected(n1, edge, n2) {
            self.graph.add_edge(n1, n2, edge);
        }
    }

    fn connect_node_to_parent(&mut self, node: AstNode<'ast>) {
    ...
    pub(super) fn with_parent(&mut self, parent: AstNode<'ast>) -> WithParent<'_, 'ast, E> {
    ...
    #[inline(always)]
    pub(super) fn with<F, Ret>(&mut self, f: F) -> Ret
    where
...
//     ($visitor:expr, $parent:expr, [$($child:expr$(,)?)*]) => {{
//         let parent = AstNode::from($parent);
//         $({
//             $visitor.create_edge(parent, AstNode::from($child));
...
//     ($visitor:expr, $parent:expr, {$($t:tt)*}) => {{
//         let parent = AstNode::from($parent);
//         let old = $visitor.current_parent;
//         $visitor.current_parent = parent;
//         {$($t)*};
//         $visitor.current_parent = old;
//     }};
//     // ($visitor:expr, $parent:expr, $f:expr) => {{
//     //     let parent = AstNode::from($parent);
//     //     let old = $visitor.current_parent;
//     //     $visitor.current_parent = parent;
//     //     $f(parent);
