...
impl Compiler {
    ...
    pub fn compile(
        ...
        self.run(|| {
            ...
            //     idents: Vec<&'a mut ::ast::Ident>,
            // }

            // impl<'ast> ecma_visit::VisitMut<'ast> for IdentVisitor<'ast> {
            //     fn visit_mut_ident(&mut self, ident: &'ast mut ::ast::Ident) {
            //         // println!("Ident(sym: {:?}, span: {:?})", ident.sym, ident.span);
            //         self.idents.push(ident)
            //     }
            // }

            // let mut a = IdentVisitor::default();

            // // V::visit_with(&program_ast, &mut a);
            // program_ast.visit_mut_with(&mut a);
...
//     // This needs updating to reflect changes i made to the ast:
//     impl Visit for NodeVisitor {
//         generate_visitors!([
//             [visit_class, Class],
// [visit_extends_clause, ExtendsClause],
