...
use crate::ident::{find_ids, Id};
use ast::*;
use ecma_visit::{as_folder, noop_visit_mut_type, Fold, VisitMut, VisitMutWith};
...
// BIG TODO:
// update resolver to match our ast. e.g. ExtendsClause, PropName, etc.
...
pub fn resolver<'a>(unresolved_mark: Mark, top_level_mark: Mark, typescript: bool) -> Resolver<'a> {
    assert_ne!(
        unresolved_mark,
        Mark::root(),
...
impl<'a> VisitMut for Resolver<'a> {
    ...
    fn visit_mut_export_named_specifier(&mut self, e: &mut ExportNamedSpecifier) {
        ...
        if self.config.handle_types {
            self.try_resolving_as_type(&mut e.orig);
        }
    }
    ...
    fn visit_mut_ts_getter_signature(&mut self, n: &mut TsGetterSignature) {
        if let PropName::Computed(key) = &mut n.key {
