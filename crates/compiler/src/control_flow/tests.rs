...
where
    F: FnMut(&ControlFlowGraph<Node<'_>, DummyAnnotation>, &AstGraph),
{
...
*/
fn get_all_edges_of_CFG(cfg: &ControlFlowGraph<Node<'_>, DummyAnnotation>) -> Vec<SimpleEdge> {
    cfg.graph
        .raw_edges()
        .iter()
        .map(|e| SimpleEdge {
...
*/
fn get_all_edges<'a>(
    cfg: &'a ControlFlowGraph<Node<'_>, DummyAnnotation>,
    start_token: Token,
    end_token: Token,
) -> Vec<SimpleEdge> {
...
*/
fn assert_no_edge(
    cfg: &ControlFlowGraph<Node<'_>, DummyAnnotation>,
    start_token: Token,
    end_token: Token,
) {
...
*/
fn assert_return_edge(cfg: &ControlFlowGraph<Node<'_>, DummyAnnotation>, start_token: Token) {
...
* from some node with the first token to the return node.
*/
fn assert_no_return_edge(cfg: &ControlFlowGraph<Node<'_>, DummyAnnotation>, start_token: Token) {
    let edges = get_all_edges_of_CFG(cfg);
    for edge in edges {
...
* @param input Input JavaScript.
*/
fn create_cfg<'ast, T>(root: T) -> ControlFlowGraph<Node<'ast>, DummyAnnotation>
where
...
fn assert_node_order(src: &str, expected: &[Token]) {
    ...
    let cfa = ControlFlowAnalysis::<'_, DummyAnnotation>::analyze(root, true);
