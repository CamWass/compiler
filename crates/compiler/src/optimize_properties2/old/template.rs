...
impl Visitor<CallId> for Resolver<'_, '_> {
    fn visit_node(&mut self, node: CallId, dependencies: &mut FxHashSet<CallId>) -> bool {
        ...
        let (steps, step_map) = match &self.call_templates[func] {
            ...
            CallTemplate::Precomputed(_, _, _) => unreachable!(),
        };
...
pub(super) enum CallTemplate {
    Steps(CallSteps),
    /// (references_environment_vars, return_ty, returned_props)
    Precomputed(bool, Option<Pointer>, PropertyAssignments),
}
    ...
    pub fn new(
        ...
        if !steps.iter().any(|s| match s {
            Step::StoreRValue(v) => match v {
                Some(RValue::Var(v)) => v.as_u32() < static_fn_data[func].param_end,
                None
                | Some(
                    RValue::NullOrVoid
                    | RValue::Object(_)
                ...
                Some(RValue::Prop(_)) => true,
            },
            Step::StoreLValue(v) => match v {
                Some(LValue::Var(v)) => v.as_u32() < static_fn_data[func].param_end,
                None => false,
            ...
            let (return_ty, prop_assignments) = compute_call(
                steps,
                map,
            ...
            // RValue only; can pre-compute.
            CallTemplate::Precomputed(references_env_vars, return_ty, prop_assignments)
        } else {
...
impl ReusableState for SimpleMachineState {
    fn reset(&mut self) {
        ...
        r_value.push(None);
        *l_value = None;
        lattice.prop_assignments.clear();
...
fn compute_call(
    ...
    invalid_objects: &mut GrowableBitSet<ObjectId>,
    fn_assignments: &HashableHashMap<VarId, Assignment>,
) -> (Option<Pointer>, PropertyAssignments) {
    let mut state = SimpleMachineState::default();

    let mut return_types = FxHashSet::default();
    let mut returned_props = PropertyAssignments::default();
            ...
            match step {
                ...
                Step::StoreLValue(value) => match value {
                    ...
                    Some(LValue::ObjectProp(obj, prop)) => {
                        let obj = match objects_map.entry(*obj) {
                            Entry::Occupied(entry) => *entry.get(),
                            Entry::Vacant(entry) => {
                                let object_id = objects.next_object_id();
                                entry.insert(object_id);
                        ...
                        machine.state.l_value =
                            Some(AssignTarget::Prop(Some(Pointer::Object(obj)), *prop));
                    }
                ...
                Step::Assign(conditional) => {
                    if let Some(slot) = &machine.state.l_value {
                        ...
                        match slot {
                            AssignTarget::Var(id) => {
                                machine.state.lattice.var_assignments.insert(*id, new);
                            }
                ...
                Step::Return => {
                    ...
                    let none_in_return_ty = return_types.contains(&ReturnTypeConstituent::Invalid);
                    match r_value {
                        Some(Pointer::Object(o)) => {
                            if invalid_objects.contains(o) {
                                ...
                                add_return_ty!(ReturnTypeConstituent::Object(o));
                                queue.push(o);
                            }
                        }
                        Some(Pointer::Union(union)) => {
                            ...
                            for constituent in union.constituents() {
                                add_return_ty!(ReturnTypeConstituent::Object(constituent));
                            }
                        }
                        Some(Pointer::Fn(_)) | None => {
                            if !none_in_return_ty {
                        ...
                        Some(Pointer::NullOrVoid) => {
                            add_return_ty!(ReturnTypeConstituent::NullOrVoid);
                        }
                    }
                    ...
                    while let Some(obj) = queue.pop() {
                        ...
                        for ((o, key), prop) in machine.state.lattice.prop_assignments.iter() {
                            if *o != obj {
                                continue;
                            }
                            ...
                            // let key = (obj, *key);
                            // let existing = returned_props.get(&key).copied();
                            // match existing {
                            //     Some(existing) => {
                            //         let union = create_union(unions, existing.rhs, prop.rhs);
                            //         returned_props.insert(key, Assignment { rhs: union });
                            //     }
                            //     None => {
                            //         returned_props.insert(key, *prop);
                            //     }
                            // }
                        }
                    }
    ...
    (unions.build_union(builder), returned_props)
}
...
impl<'ast> DataFlowAnalysis<'ast, '_> {
    fn flowThrough(
        ...
        'step: for step in machine.steps {
            match step {
                Step::StoreRValue(value) => match value {
                    None => {
                        machine.set_r_value(None);
                ...
                Step::Call => {
                    ...
                    if let CallTemplate::Precomputed(
                        references_env_vars,
                        return_value,
                        returned_props,
                    ) = &self.call_templates[func]
                    {
                        let copy_returned_props =
                            |invalid_objects: &GrowableBitSet<ObjectId>,
                             unions: &mut UnionStore,
                             lattice_elements,
                             lattice: &mut MachineLattice| {
                                for ((obj, key), prop) in returned_props.iter() {
                                    ...
                                    let new = if let Some(existing) =
                                        lattice.get(lattice_elements).prop_assignments.get(&key)
                                    {
                                        let union = create_union(unions, existing.rhs, prop.rhs);
                                    ...
                                    lattice.insert_prop_assignment(key, new, lattice_elements);
                                }
                            };
                                ...
                                CallArgs::Heap(args) => {
...

                                    {
                                        for arg in args.iter() {
                                            match arg {
                                                ...
                                                Some(Pointer::Union(union)) => {
                                                    let mut found_resolving = false;
                                                    for constituent in
                                                        self.unions[*union].constituents()
                                                    {
                                                        if constituent
                                                            == ObjectStore::RESOLVING_CALL
                                                        {
                                                            found_resolving = true;
                                                            break;
                                                        }
                                                        if !done.contains(&constituent) {
                                                            queue.push(constituent);
                                    ...
                                    while let Some(o) = queue.pop() {
                                            ...
                                            .iter()
                                        {
                                            if key.0 == o {
                                                ...
                                                match self.functions[func].arg_values.entry(*key) {
                                                    Entry::Occupied(mut entry) => {
                                    ...
                                    for (i, &arg) in args.iter().enumerate() {
                                        match self.functions[func].args.get_mut(i) {
                                            Some(existing) => {
                                                let union =
                                                    create_union(self.unions, *existing, arg);
                                                *existing = union;
                                            }
                                            None => {
                                                self.functions[func].args.push(arg);
                                    ...
                                    for i in 0..*len {
                                        match self.functions[func].args.get_mut(i) {
                                            Some(existing) => {
                                                if existing.is_some() {
                                                    let union =
                                                        create_union(self.unions, *existing, None);
                                                    *existing = union;
                                                }
                                            }
                                            None => {
