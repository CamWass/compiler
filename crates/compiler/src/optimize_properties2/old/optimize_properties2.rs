...
pub fn analyse(ast: &ast::Program, unresolved_ctxt: SyntaxContext) -> Store<'_> {
    ...
    // let mut num_r_value_only_templates = 0;

    // let mut r_value_only_templates: FxHashMap<&[function::Step], usize> = FxHashMap::default();

    // let mut num_steps: usize = 0;

    // for (func, template) in store.call_templates.iter_enumerated() {
    //     match template {
    //         CallTemplate::Steps(steps) => {
    //             num_steps += steps.steps.len();
    //             if !steps.steps.iter().any(|s| match s {
    //                 function::Step::StoreRValue(v) => match v {
    //                     Some(function::RValue::Var(v)) => {
    //                         v.as_u32() < store.static_fn_data[func].param_end
    //                     }
    //                     None
    //                     | Some(
    //                         function::RValue::NullOrVoid
    //                         | function::RValue::Object(_)
    //                         // | function::RValue::Prop(_)
    //                         | function::RValue::String
    //                         | function::RValue::Boolean
    //                         | function::RValue::Number
    //                         | function::RValue::BigInt
    //                         | function::RValue::Fn(_),
    //                     ) => false,
    //                     Some(function::RValue::Prop(_))=>true
    //                 },
    //                 function::Step::StoreLValue(v) => match v {
    //                     Some(function::LValue::Var(v)) => {
    //                         v.as_u32() < store.static_fn_data[func].param_end
    //                     }
    //                     None
    //                     // | Some(
    //                     //     function::LValue::RValueProp(_),
    //                     //     // function::LValue::RValueProp(_) | function::LValue::ObjectProp(_, _),
    //                     // )
    //                     => false,
    //                     Some(function::LValue::ObjectProp(_, _)|function::LValue::RValueProp(_)) => true,
    //                 },
    //                 function::Step::InvalidateRValue
    //                 | function::Step::InvalidateLValue
    //                 | function::Step::Assign(_) => false,
    //                 function::Step::StartCall(_) | function::Step::Call => true,
    //                 _ => false,
    //             }) {
    //                 *r_value_only_templates.entry(&steps.steps).or_default() += 1;
    //                 num_r_value_only_templates += 1;
    //             }
    //         }
    //         CallTemplate::Precomputed(_, _, _) => {}
    //     }
    // }

    // let mut r_value_only_templates = r_value_only_templates.into_iter().collect::<Vec<_>>();

    // r_value_only_templates.sort_unstable_by(|a, b| b.1.cmp(&a.1));
    // // r_value_only_templates.retain(|(_, functions)| *functions > 1);
    // dbg!(r_value_only_templates);

    // println!("r_value_only_templates: {}", num_r_value_only_templates);
    // println!("num_steps: {}", num_steps);

    // let mut templates: FxHashMap<&[function::Step], usize> = FxHashMap::default();

    // for (func, template) in store.call_templates.iter_enumerated() {
    //     *templates.entry(&template.steps).or_default() += 1;
    // }

    // let original = store.call_templates.len();

    // let mut templates = templates.into_iter().collect::<Vec<_>>();

    // let new = templates.len();

    // templates.sort_unstable_by(|a, b| b.1.cmp(&a.1));
    // templates.retain(|(_, functions)| *functions > 1);
    // dbg!(templates, original, new, original - new);
        ...
        match node.kind {
            ...
            NodeKind::CallExpr(node) => {
                ...
                if let Some(Pointer::Fn(func)) = callee {
                    ...
                    if let CallTemplate::Precomputed(references_env_vars, value, returned_props) =
                        &self.store.call_templates[func]
                    {
                        if *references_env_vars {
                            // Merge arguments.
                            match &args.finish() {
                                CallArgs::Heap(args) => {
                                    let mut done = FxHashSet::default();
                                    ...
                                    for arg in args.iter() {
                                        match arg {
                                            ...
                                            Some(Pointer::Union(union)) => {
                                                queue.extend(
                                                    self.store.unions[*union].constituents(),
                                                );
                                            }
                                            Some(Pointer::Fn(_))
                                    ...
                                    while let Some(o) = queue.pop() {
                                        ...
                                        for (key, value) in self.lattice.prop_assignments.iter() {
                                            if key.0 == o {
                                                ...
                                                match self.store.functions[func]
                                                    .arg_values
                                                    .entry(*key)
                                                {
                                    ...
                                    for (i, &arg) in args.iter().enumerate() {
                                        match self.store.functions[func].args.get_mut(i) {
                                            Some(existing) => {
                                                let union = create_union(
                                                    &mut self.store.unions,
                                                    *existing,
                                            ...
                                            None => {
                                                self.store.functions[func].args.push(arg);
                                            }
                                        }
                                    }
                                }
                                CallArgs::Invalid(len) => {
                                    for i in 0..*len {
                                        match self.store.functions[func].args.get_mut(i) {
                                            ...
                                            None => {
                                                self.store.functions[func].args.push(None);
                                            }
                                        }
                                    }
                                }
                        ...
                        // Merge property assignments.
                        for ((obj, key), prop) in returned_props.iter() {
                            if self.store.invalid_objects.contains(*obj) {
                                continue;
                            }
                            let key = (*obj, *key);
                            let new = if let Some(existing) =
                                self.lattice.prop_assignments.get(&key)
                            {
                                let union =
                                    create_union(&mut self.store.unions, existing.rhs, prop.rhs);
                                Assignment { rhs: union }
                            } else {
                                *prop
                            };
