...
pub fn collect(checker: &mut Checker, program: &CompProgram) -> ColorRegistry {
    ...
    let mut class_visitor = InstanceCollector { data: &mut data };
...
impl Data<'_> {
    ...
    fn recordType(&mut self, ty: TypeId, todo_temp_node: Option<&BoundNode>) -> ColorId {
    ...
    //         }
    //         // if let Some(symbol) = self.checker.types[ty].get_symbol() {
    //         //     let members = self.checker.getMembersOfSymbol(*symbol);
    //         //     // todo: dubious collect:
    //         //     let members = self.checker.symbol_tables[members]
    //         //         .keys()
    //         //         .filter(|&n| self.referenced_props.contains(n))
    //         //         .cloned()
    //         //         .collect::<Vec<_>>();
    //         //     self.registry.colors[color_id].ownProperties.extend(members);
    //         // }
    ...
    fn recordObjectType(&mut self, ty: TypeId) -> ColorId {
        ...
        // let symbol = self.checker.types[ty].get_symbol().unwrap();
        // let members = self.checker.getMembersOfSymbol(symbol);
        // // todo: dubious collect:
        // let members = self.checker.symbol_tables[members]
        //     .keys()
    ...
    fn addSupertypeEdges(&mut self, subType: TypeId, subTypeColor: ColorId) {
        if let Some(base_types) = self.checker.pubGetBaseTypes(subType) {
            self.registry
                ...
                .entry(subTypeColor)
                .or_default()
                .reserve(base_types.len());
            for &base_type in base_types.iter() {
                let base_type_color = self.recordType(base_type, None);
                self.registry
                    .disambiguationSupertypeGraph
                    .entry(subTypeColor)
                    .or_default()
                    .insert(base_type_color);
            }
        }
    }
}
...
// InstanceCollector needs access to instance types. Typescript has no api to directly find these,
...
struct InstanceCollector<'c, 'd> {
    data: &'d mut Data<'c>,
}

impl Visit for InstanceCollector<'_, '_> {
    fn visit_class(&mut self, node: &Rc<ast::Class>, parent: Option<BoundNode>) {
        let class_decl = parent.unwrap();
...
fn find_props(program: &CompProgram) -> AHashSet<JsWord> {
    let mut finder = PropertyReferenceFinder::default();
    program.visit_with(&mut finder, None);
    finder.props
}

#[derive(Default)]
struct PropertyReferenceFinder {
    props: AHashSet<JsWord>,
}

impl Visit for PropertyReferenceFinder {
