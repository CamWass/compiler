 ...
 */
pub fn isLocalName(node: &Identifier, node_data: &NodeData) -> bool {
    getEmitFlags(node_data).intersects(EmitFlags::LocalName)
}
...
pub fn isOuterExpression(node: &Node, kinds: Option<OuterExpressionKinds>) -> bool {
...
pub fn skipOuterExpressions(node: Expression, kinds: Option<OuterExpressionKinds>) -> Expression {
    while isOuterExpression(&node.clone().into(), kinds) {
...
pub fn skipOuterExpressionsOfNode(node: Node, kinds: Option<OuterExpressionKinds>) -> Node {
