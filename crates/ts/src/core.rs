 ...
 */
pub fn getSpellingSuggestion<'a, T, F>(name: &str, candidates: &'a [T], getName: F) -> Option<&'a T>
where
    F: Fn(&'a T) -> Option<&'a str>,
{
    todo!();
    // let name_len = name.len().try_into().unwrap();
    // let maximumLengthDifference = std::cmp::min(2_f64, f64::floor(name_len * 0.34));
    // let mut bestDistance = f64::floor(name_len * 0.4) + 1_f64; // If the best result is worse than this, don't bother.
    // let mut bestCandidate = None;
    // for candidate in candidates {
    //     if let Some(candidateName) = getName(candidate) {
    //         if f64::abs(candidateName.len() - name_len) <= maximumLengthDifference {
    //             if candidateName == name {
    //                 continue;
    //             }
    //             // Only consider candidates less than 3 characters long when they differ by case.
    //             // Otherwise, don't bother, since a user would usually notice differences of a 2-character name.
    //             if candidateName.len() < 3 && candidateName.toLowerCase() != name.toLowerCase() {
    //                 continue;
    //             }

    //             let distance = match levenshteinWithMax(name, candidateName, bestDistance - 0.1) {
    //                 Some(d) => d,
    //                 None => continue,
    //             };

    //             debug_assert!(distance < bestDistance); // Else `levenshteinWithMax` should return undefined
...
fn levenshteinWithMax(s1: &str, s2: &str, max: f64) -> Option<f64> {
    todo!();
